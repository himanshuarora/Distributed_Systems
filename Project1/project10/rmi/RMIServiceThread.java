...
/** RMI Service Thread.
    ...
    <p>
		Service threads are spawned by the listener thread on receiving a method request. The
		listener thread accepts method call from client. On receiving a method request, it creates a
		new RMI Service Thread and passes it on the socket to talk to the client.
    <p>
*/
...
public class RMIServiceThread<T> extends Thread {
        ...
        private synchronized void waitForClient() {
            ...
            try {
                ...
                /* Make sure that the calling interface sent from client is equal to or parent of the
				   interface at the skeleton.
				   If not, send back an RMI exception.
				*/
